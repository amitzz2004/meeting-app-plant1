...
const categoryConfig = {
  "Small Conference": {
    ...
    description: "Ideal for team discussions",
    getImage: (index) => roomImages[index % roomImages.length],
  },
  "Board Room": {
    name: "Board Room",
    description: "Perfect for client meetings & presentations",
    getImage: () => boardroom,
  },
  "Training Room": {
    name: "Training Room",
    description: "For Training Purposes",
    getImage: () => trainingroom,
  },
};
...
function groupRooms(rooms) {
  ...
  rooms.forEach((room) => {
    ...
    if (name.includes("small")) {
      categories["Small Conference"].push(room);
    } else if (name.includes("board")) {
      categories["Board Room"].push(room);
    } else if (name.includes("training")) {
      categories["Training Room"].push(room);
    }
  });
  ...
  return Object.keys(categories)
    ...
    .map((key) => ({
      ...
      rooms: categories[key].map((room, i) => ({
        ...room,
        image: categoryConfig[key].getImage(i),
      })),
    }));
}
      ...
      <div className="max-w-7xl mx-auto p-8 w-full">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="px-5 py-2 bg-white rounded-2xl shadow-sm text-sm font-medium text-slate-700 flex items-center gap-2">
            ...
            {loading ? "..." : `${roomCategories.length} Categories`}
          </div>
        </div>

        {loading ? (
          <div className="text-center text-slate-500 py-20">Loading rooms...</div>
        ) : (
